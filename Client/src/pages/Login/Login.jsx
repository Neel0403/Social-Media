...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        try {
            const loggedInUser = await loginAuth({ email: auth.email, password: auth.password }, dispatch);
                ...
                <div className="flex flex-col justify-center" style={{ flex: 1 }}>
                    <form onSubmit={handleLogin}>
                        <div className="bg-white h-[300px] p-[20px] rounded-md flex flex-col justify-between shadow-lg">
                            ...
                            <Link to={"/register"} className="h-[50px] w-1/2 rounded-lg bg-blue-600 hover:bg-blue-700 transition text-white text-lg font-bold self-center">
                                Create A New Account
                            </Link>
                        </div>
                    </form>
